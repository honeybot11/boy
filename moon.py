...
admx=["http://aminoapps.com/p/nn8j8r","http://aminoapps.com/p/jjx6hi","http://aminoapps.com/p/t0zzir"]
...
def on_group_member_leave(data):
	if data.comId==cidy:
		try:
			subclient.send_message(chatId=data.message.chatId,message="""[c]Rip gc ka kachara saaf hua""")
...
lis = ["It is certain",
    "It is decidedly so",
    "Without a doubt",
    "Yes definitely","May be Yes",
    "You may rely on it","Yes much",
    "As I see it yes","I think Yes",
    "Most likely","Nahi batunga",
    "Outlook good","I don't think","Nope",
    "Yes","Chup","Don't dare to ask",
    "Signs point to yes","I dont wanna say"
    "Reply hazy try again",
    "Ask again later","Abki pucha to thappd lagegi",
    "Better not tell you now",
    "Cannot predict now",
    "Concentrate and ask again",
    "Don't count on it","Moon and honey busy dont disturb ","Busy ask me later",
    "My reply is no","I said NO",
    "My sources say no",
    "Outlook not so good",
...
ndc://g/user-profile/{d}""")
			...
			if x.lower()=="-end" and c==[]:
				try:
					...
					subclient.send_message(chatId=data.message.chatId,message=f"end command will not work, pls contact 2.0 for further assistance")
					print(f"VC started")
				except Exception as e:
					print(e)
...
-alock,-rlock (admin cmds)""")
			...
			if x.lower()=="-flirt" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message="""[i] I want to hold you and then kiss you just to check if you are actually made of sugar. Because you are so sweet!""")
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
			...
			if x.lower()=="-hugme" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message="""[i] hug you it's not just a normal hug, one of those tight hugs that take my breath away. Give me butterflies, and make me smile like crazy.” (づ｡◕‿‿◕｡)づ
[i]I m here for you....""")
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
			...
			if x.lower()=="-gf" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message=""" sorry!you are not eligible for gf""")
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
			...
			if x.lower()=="-twerk" and c==[]:
				try:
					...
					print(e)
			if x.lower()=="-more" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message="""[b]Some More Commands !!
-flirt
-punchme
-twerk
-cheerme
...
-fart""")
			...
			if x.lower()=="-gn" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message=f"[ci]Good night <${data.message.author.nickname}$> !!",mentionUserIds=[data.message.author.userId])
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
			if x.lower()=="-gm" and c==[]:
				try:
					subclient.send_message(chatId=data.message.chatId,message=f"[ci]Good Morning <${data.message.author.nickname}$> !!",mentionUserIds=[data.message.author.userId])
					print(f"Info requested by {data.message.author.nickname}")
				except Exception as e:
					print(e)
